        ...
        for pos in aln.positions:
            ...
            if m_res is None:  # deletion
                ...
                try:
                    ...
                    print(f'(loopmodel) Unfreezing {resi}B')
        ...
        for pos in aln.positions:
            ...
            if m_res is None:  # deletion
                ...
                try:
                    ...
                    print(f'(automodel) Unfreezing insertion at {resi}A')
                except:
                    resrange = self.residue_range(
                        f'{resi}:B', f'{resi}:B'
                    )
                    print(f'(automodel) Unfreezing insertion at {resi}B')
                mutations.append(resrange)
                ...
                try:
                    ...
                    print(f'(automodel) Unfreezing mutation at {resi}A: {t_res.name} to {m_res.name}')
                except:  # lame, but easiest way I found since I cannot access chain id.
                    resrange = self.residue_range(
                        f'{resi}:B', f'{resi}:B'
                    )
                    print(f'(automodel) Unfreezing mutation at {resi}B: {t_res.name} to {m_res.name}')
